  ...
  if(isActive) {
    return (
      <li className='nav-item'>
        <Link href={href} className='nav-link p-2 py-1 m-1 flex items-center bg-primary text-white rounded'>
          <div className='inline-block my-auto text-lg mr-2'>{icon}</div>
  
          
          <p className={'inline-block m-0 font-medium text-grey-800'}>{label}</p>
        </Link>
      </li>
    );
  } else {
    return (
      <li className='nav-item'>
        <Link href={href} className='nav-link p-2 py-1 m-1 flex items-center hover:bg-gray-100 text-secondary rounded'>
          <div className='inline-block my-auto text-lg mr-2'>{icon}</div>
